...
if (isset($_GET['change'])){
										...
										if($status=='failed'){
											echo ('<p style="color:red">Error! Package was not change</p>');
											
										}else if($status=='OK'){
											echo ('<p style="color:green">Package(s) Successfully Changed</p>');
		...
		if ($Process != "Ok"){
				echo('<h3 style="color:red; font-family:Oswald; text-transform:uppercase;"> No Match Found! Please Try Again</h3>');
					...
					if (isset($_SESSION['resultln'])) {
						...
						echo('<tr><th>Select</th><th>First Name</th><th>Last Name</th><th>Wildcat ID</th></tr>');
						for ($i = 0; $i < sizeof($last_name_array); $i++){
							echo('<tr>');
								echo('<td><input type = "checkbox" name = "index[]" value = "'. $i .'" /></td>');
								echo('<td>'. $first_name_array[$i].'</td>');
								echo('<td>'. $last_name_array[$i].'</td>');
...
?>						
				...
				<div  id="test2">
				<form  action="php_process_files/package_registry/resident_search.php" method="post">
					<div class="test2">
					
					<fieldset class="overide" style="width:836px;">
						<legend>Search the Packet List</legend>
						<div class="col-xs-12" style="">
							
							<input type="text" size="30" style="margin-bottom:20px;" name="LName" value="" id="ID_Swipe" class="form-control textinput" onkeyup="showResult(this.value)" placeholder="Last Name">
								<input name="" onclick="changeButtonColor()" value="Last Name" type="submit" id="submit_Button" class="btn btn-primary">
								<!-- style="display: inline; background-color:#CABEDB;" -->
